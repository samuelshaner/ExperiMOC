    ...
    def makeTracks(self):
        ...
        for x in range(len(self.nx)):
            for z,nz in enumerate(self.nz):
                self.theta[x][z] = pi/2.0 - arctan(d/(nz*sqrt((w/self.nx[x]/2.0)**2+(h/self.ny[x]/2.0)**2)))
...
class Track3D(object):
    ...
    Attributes:
      r_in      = <Vector3D> object representing the starting coordinates
      r_out     = <Vector3D> object representing the ending coordinates
      track_in  = <Track3D> object that connects at end of this track
      track_out = <Track3D> object that connects at beginning of this track 
     segments  = a list of all the segments in this track
      phi       = azimuthal angle
      weight    = weight for this track
    ...
    def __init__(self, r_in, r_out, phi, theta):
        """Create a 2D track within problem domain"""
    ...
    def __repr__(self):
        return "<Track3D: ({0.x},{0.y},{0.z}) --> ({1.x},{1.y},{1.z}), phi = {2}, theta = {3}>".format(
            self.r_in, self.r_out, self.phi, self.theta)
